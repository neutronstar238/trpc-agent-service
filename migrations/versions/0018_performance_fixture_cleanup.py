"""Add a bounded performance-fixture cleanup entry point.

Revision ID: 0018_performance_fixture_cleanup
Revises: 0017_im_acceptance_evidence
...
scope is limited to an ownership-proven synthetic performance tenant.
"""
...
revision = "0018_performance_fixture_cleanup"
down_revision = "0017_im_acceptance_evidence"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.execute(
        r"""
        CREATE OR REPLACE FUNCTION public.cleanup_performance_fixture(
            p_tenant_id text,
            p_run_id text,
        ...
        BEGIN
            IF p_tenant_id IS NULL
               OR p_tenant_id !~ '^perf-[0-9a-f]{32}$' THEN
                RAISE EXCEPTION 'performance fixture tenant identity is invalid'
                    USING ERRCODE = '22023';
            END IF;

            fixture_suffix := pg_catalog.substr(p_tenant_id, 6);
            IF p_run_id IS NULL
               OR p_run_id <> 'perf-fixture-' || fixture_suffix THEN
                RAISE EXCEPTION 'performance fixture run identity is invalid'
                    USING ERRCODE = '22023';
            END IF;
            IF p_manifest_checksum IS NULL
               OR p_manifest_checksum !~ '^[0-9a-f]{64}$' THEN
                RAISE EXCEPTION 'performance fixture manifest checksum is invalid'
                    USING ERRCODE = '22023';
            END IF;
            ...
            IF EXISTS (
                ...
                RAISE EXCEPTION
                    'performance fixture tenant has an active migration write barrier'
                    USING ERRCODE = '55000';
            END IF;
            ...
            SELECT pg_catalog.count(*)
             ...
             WHERE tenant.tenant_id = p_tenant_id
               ...
               AND audit.idempotency_key =
                    p_run_id || ':tenant:' || p_manifest_checksum
               AND audit.trace_id =
                    'admin:' || p_run_id || ':tenant:' || p_manifest_checksum;
            ...
            IF deleted_count <> 1 THEN
                RAISE EXCEPTION 'performance fixture tenant cleanup was incomplete'
                    USING ERRCODE = 'P0001';
            END IF;
            deleted_counts := deleted_counts || pg_catalog.jsonb_build_object(
        ...
        ALTER FUNCTION public.cleanup_performance_fixture(text, text, text)
            OWNER TO trpc_migration;
        REVOKE ALL ON FUNCTION public.cleanup_performance_fixture(text, text, text)
            FROM PUBLIC, trpc_worker, trpc_metrics;
        GRANT EXECUTE ON FUNCTION public.cleanup_performance_fixture(text, text, text)
            TO trpc_runtime;
        """
    )
...
def downgrade() -> None:
    op.execute(
        ...
        DROP FUNCTION IF EXISTS public.cleanup_performance_fixture(text, text, text);
        """
    )
