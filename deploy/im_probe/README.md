...
1. 将 `im-probe.env.example` 复制为 host-only 的
   ...
   仓库中的 `provider_runner.py` 是可安装的 fail-closed 编排器；复制到上述路径后
   设为 root 所有、不可组/其他用户写并赋予执行权限。飞书和企微的真实 evidence
   driver 分别由 `TRPC_IM_PROBE_FEISHU_DRIVER`、`TRPC_IM_PROBE_WECOM_DRIVER`
   指定，也必须位于 checkout 外并接受独立审查。
