...
def _kubectl(
    ...
    *,
    ...
    try:
        ...
        raise PerformanceJobError("kubectl request timed out") from error
    except OSError as error:
...
def _get_job(spec: PerformanceJobSpec, job_name: str) -> Mapping[str, Any]:
    result = _kubectl(
        spec,
...
def _wait_for_job(spec: PerformanceJobSpec, job_name: str) -> Mapping[str, Any]:
    ...
    while True:
        payload = _get_job(spec, job_name)
...
def _get_named_object(spec: PerformanceJobSpec, kind: str, name: str) -> Mapping[str, Any] | None:
    result = _kubectl(
        spec,
        ["get", kind, name, "--namespace", spec.namespace, "-o", "json"],
        timeout_seconds=min(spec.cleanup_timeout_seconds, 20.0),
    )
    if result.returncode != 0:
...
def _delete_and_confirm(spec: PerformanceJobSpec, kind: str, name: str) -> dict[str, Any]:
    ...
    while True:
        observed = _get_named_object(spec, kind, name)
        if observed is None:
    ...
    try:
        ...
        if fixture_report is not None:
            try:
                ...
                if _run_child(cleanup_command, cleanup_environment, timeout) != 0:
