...
def test_runner_gets_secret_paths_but_not_secret_values(probe_fixture, monkeypatch) -> None:
    ...
    monkeypatch.setattr(im_probe.subprocess, "run", fake_run)
    assert im_probe._run_provider_runner(config, request) == {}
    environment = observed["kwargs"]["env"]
    runner_input = observed["kwargs"]["input"]
    assert all(
        str(path) in environment.values()
        for path in config.credential_paths["feishu"].values()
    )
